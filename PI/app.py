...
def init_db():
    ...
    # Migrações simples para bancos já existentes
    try:
        db.execute("ALTER TABLE users ADD COLUMN crmv TEXT")
    except sqlite3.OperationalError:
        pass
    try:
        db.execute(
            "ALTER TABLE users ADD COLUMN is_vet_verified INTEGER NOT NULL DEFAULT 0"
        )
    except sqlite3.OperationalError:
        pass
    try:
...
if __name__ == "__main__":
    if not DB_PATH.exists():
        ...
        # Garantir que as tabelas existam mesmo se o arquivo já existir
        init_db()
    seed_admin()
    app = create_app()
    app.run(debug=True)
